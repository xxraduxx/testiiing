...
CEventSignalItem::CEventSignalItem( int nUniqueIndex )
{
	m_nUniqueIndex = nUniqueIndex;
	m_nParamUniqueIndex = 0;
}

CEventSignalItem::~CEventSignalItem()
{
	SAFE_DELETE_PVEC( m_pVecList );
}


bool CEventSignalItem::AddParameter( CUnionValueProperty *pVariable, bool bAllowUnique )
{
...
void CEventSignalItem::RemoveParameter( CString &szStr )
{
...
bool CEventSignalItem::IsExistParameter( CString &szStr, CString &szOriginalName )
{
	for( DWORD i=0; i<m_pVecList.size(); i++ ) {
...
CUnionValueProperty *CEventSignalItem::GetParameterFromName( CString &szName )
{
	for( DWORD i=0; i<m_pVecList.size(); i++ ) {
...
bool CEventSignalItem::Save( FILE *fp )
{
	...
	fwrite( &m_nBackgroundSideColor, sizeof(int), 1, fp );
	fwrite( &m_nSelectColor, sizeof(int), 1, fp );
	fwrite( &m_nSelectSideColor, sizeof(int), 1, fp );
...
bool CEventSignalItem::Load( FILE *fp )
{
	...
	fread( &m_nBackgroundSideColor, sizeof(int), 1, fp );
	fread( &m_nSelectColor, sizeof(int), 1, fp );
	fread( &m_nSelectSideColor, sizeof(int), 1, fp );
...
bool CEventSignalItem::ExportHeader( FILE *fp, int& nUsingBuffCount, int &nUsing64BuffCount )
{
