...
{
	m_cfDefault.dwMask = CFM_CHARSET | CFM_FACE | CFM_SIZE | CFM_OFFSET | CFM_COLOR | CFM_ITALIC;
	m_cfDefault.dwMask ^= CFM_BOLD ^ CFM_STRIKEOUT ^ CFM_UNDERLINE;
	m_cfDefault.dwEffects = 0;
	m_cfDefault.yHeight = 170; //10pts * 20 twips/point = 200 twips
	...
	strcpy_s(m_cfDefault.szFaceName,"Courier New");
...
void CSyntaxColorizer::createTables()
{
	...
	*(m_pTableZero + '[') = CM2START; *(m_pTableOne + '"')    = DQEND;
...
void CSyntaxColorizer::AddKeyword(LPCTSTR Keyword, COLORREF cr, int grp)
{
	...
	strcpy_s(keyword, strlen(Keyword) + 1, Keyword);
...
void CSyntaxColorizer::addKey(LPCTSTR Keyword, CHARFORMAT cf, int grp) //add in ascending order
{
	...
	strcpy_s(pskNewKey->keyword,strlen(Keyword) + 1,Keyword);
